...
class StaffRepository implements IStaffRepository {
  ...
  Map<String, dynamic> _toJson(Staff staff) {
    final baseData = {
      ...
      'joinDate': staff.joinDate.toIso8601String(),
    ...
    if (staff is Doctor) {
      return {
        ...
        'appointmentIds': staff.appointmentIds,
  ...
  Future<void> _saveData() async {
    try {
      final file = File(_filePath);
      final jsonList =
          _staffStorage.values.map((staff) => _toJson(staff)).toList();
  ...
  Future<List<Staff>> getStaffByRole(String role) async {
    ...
    if (filtered.isNotEmpty) {
      print(
          '[Repository] Sample staff: ${filtered.first.name} (${filtered.first.role})');
    }
